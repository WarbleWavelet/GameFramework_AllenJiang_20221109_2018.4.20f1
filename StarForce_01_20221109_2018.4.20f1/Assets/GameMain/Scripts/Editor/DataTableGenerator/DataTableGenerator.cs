    ...
    public sealed class DataTableGenerator
    {
        ...
        public static bool CheckRawData(DataTableProcessor dataTableProcessor, string dataTableName)
        {
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                if (!NameRegex.IsMatch(name))//数据表的数据
                {
        ...
        private static string GenerateDataTableParser(DataTableProcessor dataTableProcessor)
        {
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                else
                {
                    ...
                    {
                        //对应DataTableExtension中的方法名
                        stringBuilder.AppendFormat("            {0} = DataTableExtension.Parse{1}(columnStrings[index++]);", dataTableProcessor.GetName(i), "Vector3List").AppendLine();  
                    }
                    else
                    { 
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                else
                {
                    if (dataTableProcessor.GetLanguageKeyword(i) == "List<Vector3>")
                    {
                        ...
                        stringBuilder.AppendFormat("                    {0} = binaryReader.Read{1}();", dataTableProcessor.GetName(i), "Vector3List").AppendLine();
